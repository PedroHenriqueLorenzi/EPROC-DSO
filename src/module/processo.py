...
class Processo:
    ...
    def adicionar_reu(self, reu):
        if isinstance(reu, Reu) and not self.__processo_arquivado: self.__partes.append(reu)

    def adicionar_vitima(self, vitima):
        if isinstance(vitima, Vitima) and not self.__processo_arquivado: self.__partes.append(vitima)

    def adicionar_advogado(self, advogado):
        if isinstance(advogado, Advogado) and not self.__processo_arquivado: self.__advogados.append(advogado)

    def trocar_juiz_responsavel(self, juiz): ####
        if isinstance(juiz, Juiz) and not self.__processo_arquivado: self.__juiz_responsavel = juiz

    def trocar_promotor_responsavel(self, promotor): ###
        if isinstance(promotor, Promotor) and not self.__processo_arquivado: self.__promotor_responsavel = promotor

    def trocar_tribunal_responsavel(self, tribunal): ####
        if isinstance(tribunal, Tribunal) and not self.__processo_arquivado: self.__tribunal_responsavel = tribunal




    def adicionar_acusacao(self, acusacao):
        if isinstance(acusacao, Acusacao) and not self.__processo_arquivado: self.__documentos.append(acusacao)

    def adicionar_audiencia(self, audiencia):
        if isinstance(audiencia, Audiencia) and not self.__processo_arquivado: self.__documentos.append(audiencia)

    def adicionar_defesa(self, defesa):
        if isinstance(defesa, Defesa) and not self.__processo_arquivado: self.__documentos.append(defesa)

    #def registrar_movimentacao(self, movimentacao):
        #pass

    def adicionar_sentenca(self, sentenca):
        if isinstance(sentenca, Sentenca) and not self.__processo_arquivado: self.__documentos.append(sentenca)
